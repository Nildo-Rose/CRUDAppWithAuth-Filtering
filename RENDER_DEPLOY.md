...
**If the service starts but then crashes (or you see "No open ports" after "Running ..."):** Open the deploy **Logs** and look for the real error (e.g. `better-sqlite3`, `EACCES`, or `SQLITE_READONLY`). The backend is now configured to store the SQLite DB in `/tmp` when `NODE_ENV=production`, so it can write on Render. Ensure **NODE_ENV** is set to **production** in the service Environment. If you see a native module error, Render may need a different Node version—the backend specifies **Node >= 18** in package.json.
